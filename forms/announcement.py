...
class CreateAnnouncementForm(FlaskForm):
    ...
    number_of_house = StringField("Номер дома", validators=[DataRequired(), Length(min=1, max=999)])
    price = IntegerField('Цена', validators=[DataRequired(), NumberRange(min=1)])
    announcement_type = SelectField('Тип объявления', choices=[('flat', 'Квартира'), ('house', 'Дом')],
                                     validators=[DataRequired()])
    square = FloatField('Площадь', validators=[DataRequired()])
    kitchen_square = FloatField('Площадь кухни', validators=[DataRequired()], default=1)
    ...
    number_of_floors = IntegerField('Этажи', validators=[DataRequired()], default=1)
    year_of_construction = IntegerField('Год постройки', validators=[DataRequired()], default=2000)
    photo = FileField('Фотография', validators=[FileRequired(), FileAllowed(['jpg', 'png'],
                                                                           'Только изображения')])
